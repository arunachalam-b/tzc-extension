function showTimezonePopup(selectedText) {
  ...
  addTimezone.onclick = () => {
    ...
    let newLocation = prompt("Enter location for the timezone:");
    if (newTz && newLocation) {
      let offset = parseFloat(newTz);
      let localTime = new Date(date.getTime() + offset * 3600 * 1000);
      let timeElement = document.createElement("div");
      timeElement.textContent = `${newLocation} (Custom): ${localTime.toISOString().slice(0, 19).replace("T", " ")}`;
      popup.appendChild(timeElement);
    }
  };
